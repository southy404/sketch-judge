...
function configurationErrorResult(error: unknown): ApiResult | null {
  if (!(error instanceof AiConfigurationError)) return null;
...
export function createHealthResponse() {
  ...
  try {
    ...
    // Keep health safe even when AI_PROVIDER=openrouter is missing a key.
  }
...
export async function createMotifResponse(body: unknown): Promise<ApiResult> {
  ...
  try {
    ...
    const configError = configurationErrorResult(error);
